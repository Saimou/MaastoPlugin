...
namespace MaastoPlugin
{
        ...
        , m_pickMarker( nullptr )
    {
        ...
        connect( m_polygonDrawer, &PolygonDrawer::polygonClosed, this, [this]()
        {
            ...
            if ( win && !m_polygonDrawer->getClosedVertices().empty() )
            {
                ...
                if ( mesh )
                {
                    // Jos polygoni piirrettiin View 2:ssa, pakota mesh näkymään View 1:ssä
                    // (addToDB() asettaisi displayksi aktiivisen ikkunan eli View 2:n)
                    if ( win == m_selectionGLWindow && m_cloud && m_cloud->getDisplay() )
    ...
    void MaastoDialog::processLine()
    {
        ...
        if ( mesh )
        {
            // Jos viivalinja piirrettiin View 2:ssa, pakota mesh näkymään View 1:ssä
            if ( m_workingGLWindow == m_selectionGLWindow && m_cloud && m_cloud->getDisplay() )
                mesh->setDisplay( m_cloud->getDisplay() );
    ...
    void MaastoDialog::copyLineRight()
    {
        ...
        if ( mesh )
        {
            // copyLineRight käyttää samaa m_workingGLWindow:a kuin processLine —
            // jos viivalinja piirrettiin View 2:ssa, pakota mesh näkymään View 1:ssä
            if ( m_workingGLWindow == m_selectionGLWindow && m_cloud && m_cloud->getDisplay() )
                mesh->setDisplay( m_cloud->getDisplay() );
